...
const VissimAutomotive = () => {
  ...
  const introTexts = t("vissim_automotive_introduction" as never, { returnObjects: true }) as string[];
  const introduction = introTexts.map((text) => ({ text }));

  const features = resource.vissim_automotive_features.map((feat) => {
    const imgTexts = t(`vissim_automotive_features.${feat.id}.imgTexts` as never, { returnObjects: true }) as string[] | string;
